...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='hedln-r',
                        choices=['hedln-lstm', 'hedln-r', 'hedln-cr'],
    ...
    try:
        for epoch in range(setting["n_epochs"]):
            ...
            for batch, (x, y1, y2) in enumerate(train_loader):
                ...
                y1_tgt = y1.transpose(0, 1).to(device)
                y2_tgt = y2.transpose(0, 1).to(device)

                y1_, y2_ = model(x)
                loss1 = criterion(y1_, y1_tgt)
                loss2 = criterion(y2_, y2_tgt)
                penalty_nom = torch.mean((y2_[:, :, 0] - y1_[:, :, 0]) ** 2)
                penalty_fg = torch.mean(torch.pow(
                    torch.max(torch.zeros(1).to(device), y2_[:, :, 1] - y1_[:, :, 1]),
                    2)
                )
                penalty_bg = torch.mean(torch.pow(
                    torch.max(torch.zeros(1).to(device), y2_[:, :, 2] - y1_[:, :, 1]),
                    2)
                )
                loss = loss1 + loss2 + alpha * penalty_nom + beta * penalty_fg + gamma * penalty_bg
            ...
            with torch.no_grad():
                ...
                for i, (x, y1, y2) in enumerate(val_loader):
                    ...
                    loss_val = loss1 + loss2 + alpha * penalty_nom + beta * penalty_fg + gamma * penalty_bg
    ...
    # ---------- Test ----------
        ...
        with torch.no_grad():
            ...
            for i, (x, y1, y2) in enumerate(test_loader):
                ...
                loss_test = loss1 + loss2 + alpha * penalty_nom + beta * penalty_fg + gamma * penalty_bg

                total_loss_test += loss_test

                gt1 = y1.long().squeeze().T.numpy()
                ground_truths1.append(gt1)
                gt2 = y2.long().squeeze().T.numpy()
                ground_truths2.append(gt2)

                p1 = y1_.squeeze().T.cpu().numpy()
                p1 = utils.apply_threshold1(p1)
